...
### v0.8.1 — Paramount+ Apple TV 原字幕就地翻译回退

- tvOS 忽略改写后的主清单时，不再继续改写播放元数据；保持视频、音频、DRM 与授权链原样。
- Apple TV 选择原有 English 字幕后，模块直接把 Paramount 的 WebVTT 分片转换为中英双语。
- iPhone/iPad 继续使用原有中文字幕轨道方式，不受该 tvOS 回退影响。
...
- 0.7.8 实机日志确认 `dynamicplay` 已改写，但 tvOS 没有采用该字段，也没有请求 `/manifest`，而是继续通过 Irdeto 会话恢复缓存的 AVPlayer 资源。
- `irdeto-control/session-token` 中的未签名 Paramount 主清单现在同样直接指向 `example.com/manifest`，让真正建立播放会话的最后一个清单入口也进入字幕 Gateway。
- 会话令牌、许可证、DRM、带查询参数或签名的 URL 仍保持原样；手机端字幕清单和其他平台逻辑不变。

### v0.7.8 — Paramount+ Apple TV 主清单 Gateway

- 0.7.7 实机日志确认 tvOS 会绕过播放描述和 Irdeto 响应中仅追加的版本查询参数，即使换到新剧集仍会直接从音视频分片开始。
- Apple 播放描述中的未签名 Paramount HLS 地址现在直接指向 `example.com/manifest`；Gateway 获取正式主清单、就地替换英文字幕 URI，并把所有相对音视频地址恢复为原 CDN 的绝对地址。
- Irdeto、DRM、Token 和签名 URL 保持原样；Max、Discovery+、Pluto TV 及其他平台逻辑不变。
...
- tvOS 实机日志确认 Splice 预览清单已正常解密，但正式剧集会从旧主清单直接恢复到媒体分片，导致 GSS 没有机会改写字幕 URI。
- Shadowrocket 现在额外处理精确的 `irdeto-control/session-token.json` 响应；只为已知 Paramount 媒体域名中未签名的 `.m3u8` URL 添加版本参数，强制 AVPlayer 重新请求正式主清单。
- 会话令牌、许可证地址、签名清单 URL 和其他 DRM 字段保持原样；Max、Discovery+ 及其他平台逻辑不变。
...
- 根据 Apple TV 实机 PacketTunnel 请求链确认，Paramount+ tvOS 15.23.0 使用整体平台标识 `appletvtvos`，且通过 `apps-api/.../dynamicplay/show/*.json` 取得所选节目的播放描述。
- Shadowrocket 安全规则现在仅补充上述 tvOS VOD 元数据入口；手机端原有 `video/cid/*.json` 路径继续保留。
- 登录、账户、播放历史、可播放性检查和 DRM 会话接口继续排除，避免扩大 HTTP 解密范围影响播放。
